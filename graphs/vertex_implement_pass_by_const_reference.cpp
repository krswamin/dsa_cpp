...
public:
  Vertex(unsigned int index, int value,
         const vector<unsigned int> &indices_connected_to);
  void print_vertex();
};

/*
indices is passed by const reference, so no copy is made when passing it into
the constructor. But inside the constructor the line this->indices_connected_to
= indices_connected_to; performs a copy of the entire vector into the
connected_indices member variable. So connected_indices becomes a separate
object, with its own dynamically allocated internal array, holding the same
integer values as indices, but at a different memory location.
*/
