...
void UGraph::add_node(int new_node, const vector<int>& neighbors) {
    ...
    // Add the new index as a connection to the other nodes that it is connected to
...
void UGraph::delete_edge(int idx1, int idx2){
    ...
    graph[idx1].erase(remove(graph[idx1].begin(), graph[idx1].end(), idx2), graph[idx1].end());
    
    // For idx2, check if idx1 is a prexisting connection. 
    // If the connection exists, delete it
    graph[idx2].erase(remove(graph[idx2].begin(), graph[idx2].end(), idx1), graph[idx2].end());
}
