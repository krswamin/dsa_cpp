...
int main() {
  ...
  vector<vector<int>> arrs = {{-2, -4, 1, 8, 13, 27, 8}, {-6, 6, 0}, {1, 9, 3}};

  // KSW: deliberately using 3 for loops to demonstrate the utility of using
  ...
  arrs = {{-2, -4, 1, 8, 13, 27, 8}, {1, 9, 3}, {-6, 6, 0}};
...
void delete_even_numbers_using_remove_if(vector<int> &arr) {
  // KSW: remove_if reorders the array inplace. The items to be deleted are at
  // the end It returns the new end index
  auto new_end =
      remove_if(arr.begin(), arr.end(), [](int x) { return x % 2 == 0; });
