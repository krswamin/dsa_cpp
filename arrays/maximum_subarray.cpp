...
Extend the subarray. since starting a new sum will definitely be negative
atleast until the first element but dont increment the max_end_idx

Case 3: Current sum is -ve, next integer is -ve
Start a new sub array : because If you include the next integer it is going to
make the sum worse but which will be the max sub array, the previous or the
current one ?? lets say current sum is -2, next integer is -15. In this case the
max sum will be current sum lets say current sum is -15, next integer is -15. In
this case the max sum will be current sum
...
Instead of coding such a complicated logic. there is a more succinct way to do
this Check the code
...
int main() {
  vector<vector<int>> arrays = {{-1, 10, 8, 7, -6, 6}, {2, 3, -8, 7, -1, 2}};
  vector<int> result;
  ...
  for (vector<int> a : arrays) {
    ...
    cout << "\n Max_sum = " << result[0] << ", start_idx = " << result[1]
         << ", end_idx =" << result[2];
    cout << "\n";
  }
}
