...
Handle the case when xmax=smax and then the number you are examining is above or below that
*/
#include<iostream>
#include<vector>
using namespace std;
...
bool second_maximum(const vector<int> &arr, int &smax){
    ...
    for(auto x:arr){
        ...
        // note x<smax condition here is redundant because it would reach here only if x<smax
        else if(max==smax ){
            smax = x;
        }
    }
