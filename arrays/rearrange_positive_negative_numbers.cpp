...
int main() {
  vector<vector<int>> arrs = {{5, -2, 7, 3, 0, 8, 0, -6},
                              {-1, 10, 8, 7, -6, -6},
                              {-2, -3, -8, 7, -1, 2},
                              {0, 1, -2, -3, -4, -5, -6, -7}};

  // KSW: L1
  // Deliberately using three for loops to demonstrate the need to use
  ...
  cout << "Array after rearrange: (nothing has been rearranged since auto "
          "deduces by value)\n";
  for (auto arr : arrs) {
    print_array(arr);
  }
  ...
  cout << "Array after  rearrange: (array has been rearranged since auto uses "
          "reference)\n";
  for (auto &arr : arrs) {
    print_array(arr);
  }
