...
Stack 1 stack<Vertex*>      : for Parent. stack of pointers to vertices. Pointers make it efficient
Stack 2 stack<unsigned int> : for child-index-position
Array   vector<Vertex*>     : for visited nodes. stack of pointers to visited vertices Pointers make it efficient
current_vertex: which tracks the top of the parent stack
...
class Tree{
    ...
    public:
    Tree(); 
    Tree(const vector<unsigned int>& indices,
...
Tree::Tree(const vector<unsigned int>& indices, 
             const vector<int>& values, 
             const vector<optional<vector<unsigned int>>>&  connections){
...
void Tree:: dfs(){
   ...
   while(!parent.empty()){
    ...
    current_vertex_indices_connected = (*current_vertex).get_indices_connected_to();
    // Add the current vertex to visited stack only if it is the first time you are encountering it
    // I.e before checking for children. 
    ...
    // Check for children
    // If vertex is not a leaf i.e  has children add the children to the child_index_position stack
    if (current_vertex_indices_connected){    
        ...
        // parent stack: push the new child 
        else if (child_index_position.top() !=(*current_vertex_indices_connected).size()-1) {
        ...
        // i.e. child_index_position.top() ==(*current_vertex_indices_connected).size()-1
        // parent stack: pop the parent
        // child index position stack: pop the corresponding child index position
        else {
...
// & indicates: it is a reference to a vector of Vertex pointers( prevents making copies)
// const indicates that you will not modify anything
